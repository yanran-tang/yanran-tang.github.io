...
&=\mathbb{E}_{\mathbb{P}}\left[\log \frac{e^{T}d\mathbb{Q}}{\mathbb{E}_{\mathbb{Q}}[e^{T}]d\mathbb{Q}}\right], \text{ (group log terms together)} \\
...
where $$\mathcal{F}$$ is an arbitrary of the transformation function (neural networks are functions).
...
$$
\begin{equation}
I \widehat{(X ; Z)}_{n}=\sup _{\theta \in \Theta} \mathbb{E}_{\mathbb{P}_{X Z}^{(n)}}[T_{\theta}]-\log (\mathbb{E}_{\mathbb{P}_{X}^{(n)} \otimes \hat{P}_{Z}^{(n)}}[e^{T_{\theta}}]),
\end{equation}
$$
