...
*where $$Z=(X-\mu) / \sigma$$. In particular, $$\mathbb{P}(|Z|>2) \leq 1 / 4 \text { and } \mathbb{P}(|Z|>3) \leq 1 / 9$$.*

***Proof***.

$$
\mathbb{P}(|X-\mu| \geq t)=\mathbb{P}\left(|X-\mu|^{2} \geq t^{2}\right) \stackrel{Markov}{\leq} \frac{\mathbb{E}(X-\mu)^{2}}{t^{2}}=\frac{\sigma^{2}}{t^{2}}.
$$

Let $$t=k\sigma$$,

$$
\mathbb{P}(|Z| \geq k)=\mathbb{P}(|X-\mu| \geq k\sigma)\leq\frac{1}{k^2}.
$$
